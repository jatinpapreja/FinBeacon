# Welcome, the-elevate-crew, to the 2025 TDI Global Hackathon!
...
1. This **[GitHub repository](https://github.com/db-hackathon/the-elevate-crew)** which you can use to store your code. :bookmark_tabs:
1. A **Terraform Cloud** workspace [see here](https://app.terraform.io/app/db-hackathon-2025/workspaces/hack-team-the-elevate-crew) allowing you to deploy with [Terraform](https://developer.hashicorp.com/terraform/intro) into [GCP](https://cloud.google.com/docs/terraform). :hammer:
...
You have an **infrastructure SA** `infrastructure@hack-team-the-elevate-crew.iam.gserviceaccount.com` with the same IAM permissions as team members.
...
This SA is also used when you _provision infrastructure_ using your [Terraform Cloud workspace](https://app.terraform.io/app/db-hackathon-2025/workspaces/hack-team-the-elevate-crew).

You also have a **workload SA** `workload@hack-team-the-elevate-crew.iam.gserviceaccount.com` that you can use to attach to your workloads (e.g. Cloud Run revisions).
...
Cloud Function) you must attach your **Workload SA** `workload@hack-team-the-elevate-crew.iam.gserviceaccount.com` , usually referred to in the GCP documentation as _"attaching a custom SA"_.

Both your [GitHub Actions workflows](./.github/workflows/) and [Terraform Cloud workspace](https://app.terraform.io/app/db-hackathon-2025/workspaces/hack-team-the-elevate-crew) have pre-populated variables containing the Workload
SA email.
...
* [Cloud Build](https://cloud.google.com/build?hl=en)
    * [gcloud](https://cloud.google.com/build/docs/securing-builds/configure-user-specified-service-accounts)
        * Ensure you specify
          the [--service-account=workload@hack-team-the-elevate-crew.iam.gserviceaccount.com](https://cloud.google.com/sdk/gcloud/reference/builds/submit#--service-account)
...
* [Vertex AI generally](https://cloud.google.com/vertex-ai/docs/general/custom-service-account#attach)
    * Vertex AI's Console UI does not always expose the field necessary to specify a user managed SA.  
      ...
      (typically `"serviceAccount": "workload@hack-team-the-elevate-crew.iam.gserviceaccount.com"`, to be added as a peer of the `"name": "..."`
...
```shell
PROJECT_ID="hack-team-the-elevate-crew"

curl \
-X POST \
-H "Authorization: Bearer $(gcloud auth print-access-token)" \
-H "Content-Type: application/json; charset=utf-8" \
https://europe-west4-aiplatform.googleapis.com/v1/projects/hack-team-the-elevate-crew/locations/europe-west4/pipelineJobs?pipelineJobId=tune-large-model-$(date +"%Y%m%d%H%M%S") -d \
$'{
    "displayName": "tune-large-model",    
    "serviceAccount": "workload@hack-team-move.iam.gserviceaccount.com", 
    "runtimeConfig": {
        "gcsOutputDirectory": "gs://artifacts.hack-team-the-elevate-crew.appspot.com/Foo/",
        "parameterValues": {
            "location": "us-central1",
            "project": "hack-team-the-elevate-crew",
            "large_model_reference": "text-bison@001",
            "model_display_name": "mytunedmodel_1",
            "train_steps": 100,
            "encryption_spec_key_name": "",
            "dataset_uri": "gs://artifacts.hack-team-the-elevate-crew.appspot.com/Bar.jsonl",
            "evaluation_data_uri": "",
...
2. Configure your default Cloud Shell Google Cloud Project by running `gcloud config set project hack-team-the-elevate-crew`

#### [GCP Example Apps](https://github.com/db-hackathon/support/tree/main/google-examples) :globe_with_meridians:

TBC

## [GitHub](https://github.com/db-hackathon/the-elevate-crew) :bookmark_tabs:
...
populated for you:
* vars.INFRA_SA_EMAIL - The email address representation of the SA you can use to deploy infrastructure. It has the same access rights as human team members.: infrastructure@hack-team-the-elevate-crew.iam.gserviceaccount.com
* vars.INFRA_SA_ID - The fully qualified ID representation of the SA you can use to deploy infrastructure.: projects/hack-team-the-elevate-crew/serviceAccounts/infrastructure@hack-team-the-elevate-crew.iam.gserviceaccount.com
* vars.PROJECT_ID - Your team's GCP Project ID.: hack-team-the-elevate-crew
* vars.PROJECT_NUMBER - Your teams' GCP Project Number.: 774799025602
...
* vars.WORKLOAD_SA_EMAIL - The email address representation of the SA you can attach to your workloads (e.g. to a Cloud Run service). : workload@hack-team-the-elevate-crew.iam.gserviceaccount.com
* vars.WORKLOAD_SA_ID - The fully qualified ID representation of the SA you can attach to your workloads (e.g. to a Cloud Run service). : projects/hack-team-the-elevate-crew/serviceAccounts/workload@hack-team-the-elevate-crew.iam.gserviceaccount.com
...
## [Terraform Cloud](https://app.terraform.io/app/db-hackathon-2025/workspaces/hack-team-the-elevate-crew)
...
populated for you:
* infra_sa_email - The email address representation of the SA you can use to deploy infrastructure. It has the same access rights as human team members.: infrastructure@hack-team-the-elevate-crew.iam.gserviceaccount.com
* infra_sa_id - The fully qualified ID representation of the SA you can use to deploy infrastructure.: projects/hack-team-the-elevate-crew/serviceAccounts/infrastructure@hack-team-the-elevate-crew.iam.gserviceaccount.com
* project_id - Your team's GCP Project ID.: hack-team-the-elevate-crew
* project_number - Your teams' GCP Project Number.: 774799025602
...
* workload_sa_email - The email address representation of the SA you can attach to your workloads (e.g. to a Cloud Run service). : workload@hack-team-the-elevate-crew.iam.gserviceaccount.com
* workload_sa_id - The fully qualified ID representation of the SA you can attach to your workloads (e.g. to a Cloud Run service). : projects/hack-team-the-elevate-crew/serviceAccounts/workload@hack-team-the-elevate-crew.iam.gserviceaccount.com
